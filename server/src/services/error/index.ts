...
export default class ErrorService {
	...
	public ValidationError(message?: string | ValidationError[]) {
		if (typeof message === "string" || typeof message === "undefined") {
			return this.throwApolloError(
				new ValidationError(message || "Invalid Data ! Please check your mutation and try again.")
			);
		}

		let msg: string = "";
		message.forEach((e, i) => {
			msg = msg.concat(`Property ${e.property}: `);
			if (e.constraints) {
				msg = msg.concat(Object.values(e.constraints).join(","));
			}
			msg = i !== message.length - 1 ? msg.concat(" & ") : msg.concat("");
		});

		return this.throwApolloError(new ValidationError(msg));
	}

	public UserInputError(message?: string) {
		return this.throwApolloError(
			new UserInputError(message || "Invalid Input ! Please check your mutation and try again.")
		);
	}
